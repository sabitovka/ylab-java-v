...
/**
 * Основной сервлет, который выполняет роль агрегатора REST-запросов.
 * Каждый запрос на адрес /api/* будет перехвачен.
 * Сервлет выполняет сопоставление запроса с известными ему контроллерами.
 * Если находит нужный контроллер, ищет соответствующий метод и вызывает его.
 */
@WebServlet("/api/*")
